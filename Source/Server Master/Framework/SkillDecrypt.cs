...
{
    internal class SkillDecrypt
    {
        private readonly byte[] Hash_Table_1 = new byte[]
        {
        ...
        private readonly byte[] Hash_Table_2 = new byte[]
        {
        ...
        public void ReadAndDecryptFile(string FilePath)
        {
            try
            {
                using (var br = new BinaryReader(File.Open(FilePath, FileMode.Open), Encoding.Unicode))
                {
                    var encMark = Encoding.Unicode.GetBytes("//encrypt".ToCharArray());

                    var fileLen = br.BaseStream.Length;
                    try
                    {
                        buffer = ExtensionMethods.Combine(br.ReadBytes((int)fileLen), Encoding.Unicode.GetBytes(Environment.NewLine), encMark);
                    }
                    catch (Exception)
                    {
